...
// Funzione per ottenere l'orario corrente come stringa
function getCurrentTimestamp() {
  const now = new Date();
  return now.toISOString(); // Esempio: "2024-11-18T14:30:00.000Z"
}
...
async function getChatGPTResponse(userMessage) {
  try {
    ...
    console.log(`[${getCurrentTimestamp()}] Token utilizzati:`, response.usage.total_tokens);
    return response.choices[0].message.content;
  } catch (error) {
    console.error(`[${getCurrentTimestamp()}] Errore nell'invio della richiesta a OpenAI:`, error);
...
client.on("qr", (qr) => {
  console.log(`[${getCurrentTimestamp()}] Generazione QR Code:`);
  qrcode.generate(qr, { small: true });
});

client.on("ready", () => {
  console.log(`[${getCurrentTimestamp()}] Client is ready!`);
});
...
client.on("message_create", async (message) => {
  ...
  console.log(`[${getCurrentTimestamp()}] Messaggio ricevuto:`, message.body);
  ...
  try {
    ...
    console.log(`[${getCurrentTimestamp()}] Risposta inviata:`, reply);
    await client.sendMessage(message.from, reply);
  } catch (error) {
    console.error(`[${getCurrentTimestamp()}] Errore durante la gestione del messaggio:`, error);
...
client.on("group_join", async (notification) => {
  console.log(`[${getCurrentTimestamp()}] Aggiunto a un gruppo:`, notification);

  // Esci dal gruppo immediatamente
  const groupId = notification.id.remote;
  await client.leaveGroup(groupId);
  console.log(`[${getCurrentTimestamp()}] Uscito dal gruppo:`, groupId);
});
